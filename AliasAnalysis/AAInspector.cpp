...
struct AAInspectorPass : public PassInfoMixin<AAInspectorPass> {
  PreservedAnalyses run(Function& F, FunctionAnalysisManager& AM) {
    errs() << "Function: " << F.getName() << " (" << F.getInstructionCount()
           << " instructions)\n";
    ...
    for (Instruction& I1 : instructions(F)) {
      ...
      for (Instruction& I2 : instructions(F)) {
        ...
        if (R == AliasResult::NoAlias)
          continue;  // skip trivial NoAlias to reduce noise
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "AAInspector", "v2.0", [](PassBuilder& PB) {
